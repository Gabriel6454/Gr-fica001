  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-200 selection:bg-sky-500/30 flex flex-col md:flex-row overflow-x-hidden antialiased" style={{ zoom: settings.systemScale || 1 }}>
      ...
      <header className="lg:hidden flex items-center justify-between px-4 bg-[#020617]/85 backdrop-blur-xl border-b border-white/5" style={{ height: '3.75rem', paddingTop: 'env(safe-area-inset-top)', position: 'fixed', top: 0, left: 0, right: 0, zIndex: 80 }}>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`p-2 rounded-xl transition-all ${isChatOpen ? 'bg-sky-500 text-white' : 'text-slate-400 hover:text-white'}`}
            title="Atlas AI"
          >
      ...
      <button
        onClick={() => setIsChatOpen(o => !o)}
        title="Atlas AI"
        className={`hidden lg:flex fixed bottom-6 right-6 z-[150] w-14 h-14 rounded-2xl items-center justify-center shadow-2xl transition-all duration-300 active:scale-95 hover:scale-105 ${
          isChatOpen
            ? 'bg-rose-500 shadow-rose-500/30 rotate-12'
            : 'bg-gradient-to-br from-sky-500 to-indigo-600 shadow-sky-500/40'
        }`}
      >
        {isChatOpen ? (
